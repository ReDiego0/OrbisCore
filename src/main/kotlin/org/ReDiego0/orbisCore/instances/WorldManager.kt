...
class WorldManager(private val plugin: OrbisCore) {
    ...
    fun createInstanceWorld(templateName: String, instanceId: String): World? {
        ...
        try {
            ...
            return plugin.server.createWorld(creator)

        } catch (e: Exception) {
            plugin.logger.severe("Error creando instancia: ${e.message}")
            return null
        }
    }
